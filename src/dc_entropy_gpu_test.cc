...
TEST(DcEntropyGpu, Dct8OnlyMatchesReference) {
...
TEST(DcEntropyGpu, MixedSingleGroupMatchesReference) {
    check_matches_reference(make_mixed_dc_frame(256, 256));
}

TEST(DcEntropyGpu, MixedMultiGroupMatchesReference) {
    check_matches_reference(make_mixed_dc_frame(2560, 256));
}

TEST(DcEntropyGpu, MixedGroupGridPartialEdges) {
    // 2560x2304 -> 320x288 blocks -> 2x2 DC groups, right column 64 wide and
    // bottom row 32 tall (both partial).
    check_matches_reference(make_mixed_dc_frame(2560, 2304));
}

TEST(DcEntropyGpu, Deterministic) {
    const FrameCoefficients fc{make_mixed_dc_frame(2560, 256)};
    const DcReference ref{reference_dc_encode(fc)};
    const std::size_t blocks{(fc.width / 8) * (fc.height / 8)};
    const std::vector<std::int32_t> quant_field(blocks,
                                                static_cast<std::int32_t>(fc.raw_quant_field));
    ...
    DcDeviceResult b{};
    ASSERT_TRUE(dc_encode_device(compact_dc(fc), fc.acs, fc.ytox_map, fc.ytob_map, quant_field,
                                 fc.width, fc.height, ref, a));
    ASSERT_TRUE(dc_encode_device(compact_dc(fc), fc.acs, fc.ytox_map, fc.ytob_map, quant_field,
                                 fc.width, fc.height, ref, b));
