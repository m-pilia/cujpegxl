...
bool encode_frame_m3(const std::int16_t* ac_device, const std::int32_t* dc_device,
    ...
    if (!ac_build_histogram_m3(ac_device, acs, width, height, d_ac_hist) ||
        !dc_build_histograms(dc_device, width, height, d_dc_hist) ||
    ...
    std::size_t ac_total{0};
    std::size_t dc_total{0};
    if (!ac_encode_groups_m3(ac_device, acs, width, height, d_ac_depth, d_ac_bits, d_ac_body,
                             ac_capacity, d_ac_sizes, d_ac_offsets, &ac_total)) {
        return false;
    }
