...
// over every group. `ac` is the covered-block coefficient buffer (device; three
// channel-major planes X, Y, B of (width/8 * height/8) * COEFFS_PER_BLOCK int16)
// and `acs` the per-8x8 transform signal (nullptr = all 8x8 blocks). Each
// first-block tokenizes order[covered..size) of its size gathered via
// covered_plane_slot; covered blocks contribute nothing. `histogram` is a device
// buffer of AC_HISTOGRAM_SIZE uint32 (zeroed by the call). Deterministic (integer
// atomics). Returns false on a CUDA error.
...
// writer byte-for-byte. `ac`/`acs` are the covered-block buffer and transform
// signal (see ac_build_histogram). `depth`/`bits` are the shared prefix code
// built from the phase-1 histogram. A CUB exclusive scan of the per-group byte
// sizes fixes each group's offset; `group_sizes`, `group_offsets` (num_groups
// entries, device) and `total_bytes` (host) receive the layout. Fails if
// `out_capacity` is exceeded. Deterministic. All device pointers except
// `total_bytes`.
