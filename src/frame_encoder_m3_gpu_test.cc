...
// GPU validation of the M3 encode path (encode_nv12_m3): mixed {8,16,32} blocks
...
std::vector<std::uint8_t> encode(const DeviceNv12& nv12, std::size_t w, std::size_t h) {
    ...
    EXPECT_TRUE(encode_nv12_m3(nv12.luma, nv12.luma_pitch, nv12.chroma, nv12.chroma_pitch, w, h, 0,
                               1.0f, qp, file, nullptr));
    return file;
}

class M3Ladder : public testing::TestWithParam<std::pair<std::size_t, std::size_t>> {};

TEST_P(M3Ladder, DecodesWithLibjxl) {
...
INSTANTIATE_TEST_SUITE_P(Ladder, M3Ladder,
...
// The mixed-block + CfL path must actually differ from the DCT8-only M2 path.
TEST(M3Encode, DiffersFromM2) {
    const std::size_t w{1280};
    const std::size_t h{720};
    DeviceNv12 nv12{};
    ASSERT_TRUE(upload_nv12(w, h, nv12));
    const bitstream::QuantParams qp{quant_params_for_distance(1.0f)};
    std::vector<std::uint8_t> m3{};
    std::vector<std::uint8_t> m2{};
    ASSERT_TRUE(encode_nv12_m3(nv12.luma, nv12.luma_pitch, nv12.chroma, nv12.chroma_pitch, w, h, 0,
                               1.0f, qp, m3, nullptr));
    ASSERT_TRUE(encode_nv12(nv12.luma, nv12.luma_pitch, nv12.chroma, nv12.chroma_pitch, w, h, 0,
                            1.0f, qp, m2, nullptr));
    EXPECT_NE(m3, m2);
}
