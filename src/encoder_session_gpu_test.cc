...
bool upload_frame(std::size_t width, std::size_t height, DeviceNv12& frame) {
    ...
    for (std::size_t y{0}; y < height; ++y) {
        for (std::size_t x{0}; x < width; ++x) {
            luma[y * width + x] =
                static_cast<std::uint8_t>(32 + (x * 127 / width + y) % 192);
        }
    }
    ...
    return cudaMemcpy(frame.luma, luma.data(), luma.size(),
                      cudaMemcpyHostToDevice) == cudaSuccess &&
           cudaMemcpy(frame.chroma, chroma.data(), chroma.size(),
                      cudaMemcpyHostToDevice) == cudaSuccess;
}

EncoderInput input_for(const DeviceNv12& frame, std::size_t width,
...
TEST(EncoderSessionGpu, DepthOneMatchesSynchronousInterface) {
    ...
    ASSERT_TRUE(encode_nv12(frame.luma, width, frame.chroma, width, width,
                            height, 0, 1.0f,
                            quant_params_for_distance(1.0f), synchronous));

    std::unique_ptr<EncoderSession> session{EncoderSession::create(
        EncoderConfig{.device_ordinal = 0,
                      .max_width = width,
                      .max_height = height,
                      .pipeline_depth = 1,
    ...
    ASSERT_TRUE(session->try_encode(input_for(frame, width, height, 17), future));
...
TEST(EncoderSessionGpu, BoundedPipelineCompletesDeterministically) {
    ...
    for (std::size_t i{0}; i < depth; ++i) {
        ASSERT_TRUE(session->try_encode(input_for(frame, width, height, i),
                                        futures[i]));
    }
    EncodedFrameFuture rejected{};
    EXPECT_FALSE(session->try_encode(input_for(frame, width, height, depth),
                                     rejected));
